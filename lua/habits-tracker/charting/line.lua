...
local function interpolate_approx(data, max_rows, max_height_value, x_min, x_max)
	...
	-- Calculate the heights of "point"
	local points = {}
	for _, point in ipairs(filled_data) do
		local height = math.floor((point.value / max_value) * scaling_factor)
		table.insert(points, { height = height, x = point.x, type = point.type, value = point.value })
	end

	return points
end

local function set_x_label(points)
	if #points == 0 then
		return
	end
	local same_year = true
	local same_month = true
	local year = string.sub(points[1].x, 1, 4)
	local month = string.sub(points[1].x, 6, 7)
...
---
function M.get_lines(points, max_rows, show_legend, max_height_value, title)
	local display = {}
	local lines = {}
	set_x_label(points)
	...
	if M.config and M.config.show_oxy then
		table.insert(lines, string.rep(hor, label_len) .. cross .. string.rep(hor, #points * x_label_value_len))
	end
...
function M.chartline(chart_data, add_to_buffer)
	...
	local lines =
		M.get_lines(bars, chart_data.max_rows, M.config.show_legend, chart_data.max_height_value, chart_data.title)
